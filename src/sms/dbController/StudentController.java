...
public class StudentController {
    ...
    public static Student searchStudent(Integer adNo) throws ClassNotFoundException, SQLException {
        ...
        if (rst.next()) {
            Student s = new Student(rst.getInt(1), rst.getString(2), rst.getString(3), rst.getString(4),
    ...
    public static int deleteStudent(String adNo) throws ClassNotFoundException,
            SQLException {
    ...
    public static int updateStudent(Student student) throws ClassNotFoundException, SQLException {
        String sql = "UPDATE students SET adNo= ? ,fullName= ? ,name= ? ,dob= ? ,doa= ? ,gender= ? ,grade=? ,parentName= ? ,nic= ? ,phone= ? ,address=? WHERE adNo= '"
                + student.getAdNo() + "'";
        Connection conn = DBConnection.getDBConnection().getConnection();
        PreparedStatement stm = conn.prepareStatement(sql);
        stm.setObject(1, student.getAdNo());
    ...
    public static int updateLeftStudent(Student student) throws ClassNotFoundException, SQLException {
        String sql = "UPDATE leftstudents SET adNo= ? ,fullName= ? ,name= ? ,dob= ? ,doa= ? ,gender= ? ,grade=? ,parentName= ? ,nic= ? ,phone= ? ,address=? WHERE adNo= '"
                + student.getAdNo() + "'";
        Connection conn = DBConnection.getDBConnection().getConnection();
        PreparedStatement stm = conn.prepareStatement(sql);
        stm.setObject(1, student.getAdNo());
