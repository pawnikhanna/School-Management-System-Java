...
public class StaffController {
    ...
    public static Staff searchStaff(Integer empNo) throws ClassNotFoundException, SQLException {
        ...
        if (rst.next()) {
            Staff staff = new Staff(rst.getInt(1), rst.getString(2), rst.getString(3), rst.getString(4),
    ...
    public static int deleteStaff(Integer empNo) throws ClassNotFoundException,
            SQLException {
    ...
    public static int updateStaff(Staff staff) throws ClassNotFoundException, SQLException {
        String sql = "UPDATE staffs SET empNo= ? ,teacherName= ? ,nic= ? ,dob= ? ,doa= ? ,gender= ? ,email=? ,assumpOfDuties= ? ,phone= ? ,address= ? ,incDate=? ,prsntGrade=? WHERE empNo= '"
                + staff.getEmpNo() + "'";
        Connection conn = DBConnection.getDBConnection().getConnection();
        PreparedStatement stm = conn.prepareStatement(sql);
        stm.setObject(1, staff.getEmpNo());
